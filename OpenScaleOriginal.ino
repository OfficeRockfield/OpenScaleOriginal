...
String setting_feed_name;
byte setting_feed;
boolean setting_flowrate_enable;
...
const byte escape_character = 'p'; //This is the ASCII character we look for to break reporting
...
void loop()
{
  ...
  for (int t = 0; t < 4; t++)
  {
    if (displayReading < pow(10, t) || (int)displayReading == 0)
      Serial.print(B0);
  }
  if((int)displayReading >= 1)
    Serial.print(displayReading, 0);
  Serial.print(F("&"));
  units();
  Serial.print(F("*"));
  ...
  if (setting_flowrate_enable == true)
  {
    if (count == 10)
    {
      flowrate_reading = (previousReading - currentReading) * 360;
      count = 0;
    }
    else if (count == 1)
    {
    ...
    float displayFlowrate = flowrate_reading*1000;
    for (int t = 0; t < 4; t++)
    {
      if ((int)displayFlowrate < pow(10, t))
        Serial.print(B0);
    }
    if((int)displayFlowrate >= 1 && (int)displayFlowrate < 9999) 
      Serial.print((int)displayFlowrate);
    Serial.print(F(","));
...
void units(void) {
  if (setting_units == UNITS_LBS) Serial.print(F("mls"));
  if (setting_units == UNITS_KG) Serial.print(F("kg"));
}

/*char checkFeedName(void) {
  if (setting_feed == 1) return ("Water");
  if (setting_feed == 2) return ("Perative");
  if (setting_feed == 3) return ("Nutrison");
  if (setting_feed == 4) return ("Other");
}*/

char checkFeedName(void) {
  if (setting_feed == 1) return ('W');
  if (setting_feed == 2) return ('P');
  if (setting_feed == 3) return ('N');
  if (setting_feed == 4) return ('O');
}
...
void system_setup(void)
{
  while (1)
  {
    ...
    Serial.print(F("m) Change feed type ["));
    Serial.print(checkFeedName());
    Serial.println(F("]"));

    Serial.print(F("f) Set Flowrate ["));
    Serial.print(setting_flowrate);
    Serial.println(F("mls/h]"));

    Serial.print(F("g) Enable Flowrate [O"));
    if (setting_flowrate_enable == true) Serial.print(F("n"));
    else Serial.print(F("ff"));
    Serial.println(F("]"));
    ...
    else if (command == 'g')
    {
      ...
      {
        Serial.println(F("ff"));
        setting_flowrate_enable = false;
      }
      else
      {
        Serial.println(F("n"));
        setting_flowrate_enable = true;
        count = 0;
      }
      record_system_settings();
    }
...
void flowrate_setup(void)
{
  ...
  Serial.println(setting_flowrate);
...
void feed_choose(void)
{
  ...
  {
    Serial.println(F("\r\nChoose feed type:"));
    Serial.println(F("1) Water"));
    Serial.println(F("2) Perative"));
    Serial.println(F("3) Nutrison"));
    Serial.println(F("4) Other"));
    Serial.println(F("x) Exit"));
    Serial.print(F(">"));
    ...
    if (feed == '1' || feed == '2' || feed == '3' || feed == '4' || feed == escape_character)
    {
      break;
    }
    else
    {
      Serial.println(F("Enter a valid option"));
    }
  }
...
int calcMinimumReadTime(void)
{
  ...
  if (setting_units == UNITS_LBS) characters += strlen("mls");
...
void set_default_settings(void)
{
  ...
  setting_feed_name = "Water";

  setting_feed = '1';

  setting_flowrate = 150;
...
void record_system_settings(void)
{
  ...
  EEPROM.write(LOCATION_FEED, setting_feed);

  EEPROM.write(LOCATION_FLOWRATE, setting_flowrate);

  EEPROM.write(LOCATION_FLOWRATE_ENABLE, setting_flowrate_enable);

  //writeBytes(LOCATION_FLOWRATE, setting_flowrate, sizeof(setting_flowrate));
}
...
void readSystemSettings(void)
{
  ...
  setting_feed = EEPROM.read(LOCATION_FEED);
  if (setting_feed < '1' || setting_feed > '4')
  {
    setting_feed = '4'; //Default to !
    EEPROM.write(LOCATION_FEED, setting_feed);
  }

  setting_flowrate = EEPROM.read(LOCATION_FLOWRATE);
  if (setting_flowrate < 50 || setting_flowrate > 250)
  {
    setting_flowrate = 150; //Default to !
    EEPROM.write(LOCATION_FLOWRATE, setting_flowrate);
  }

  setting_flowrate_enable = EEPROM.read(LOCATION_FLOWRATE_ENABLE);
  if (setting_flowrate_enable > 1)
  {
    setting_flowrate_enable = true; //Default to true
    EEPROM.write(LOCATION_FLOWRATE_ENABLE, setting_flowrate_enable);
  }
}
